    ...
    public class InputSettings : ScriptableObject
    {
        ...
        public sealed class ActionData
        {
            ...
            public sealed class ActionAxis
            {
                ...
                [SerializeField]
                private EAxisSource axisSource = EAxisSource.CustomAxis;
                ...
                [SerializeField]
                private EAxisEventsClamp axisStateClamp = EAxisEventsClamp.All;

                private EAxisState axisState = EAxisState.NONE;
                private bool touchDown = false;

                private int 
                    pressedFrame = -1 //( pressedFrame == Time.frameCount - 1 )
                    , releasedFrame = -1; //( releasedFrame == Time.frameCount - 1 )
                ...
                public EAxisState getAxisState
                {
                    get
                    {                
                        float currentValue = ( axisSource == EAxisSource.CustomAxis ) ? GetAxis( axisName ) : Input.GetAxis( axisName );

                        bool positiveOrNotClamped = ( axisStateClamp == EAxisEventsClamp.OnlyPositive || axisStateClamp == EAxisEventsClamp.All );
                        bool negativeOrNotClamped = ( axisStateClamp == EAxisEventsClamp.OnlyNegative || axisStateClamp == EAxisEventsClamp.All );
                        int frameCount = Time.frameCount - 1;

                        if( positiveOrNotClamped && currentValue > threshold )
                        {
                        ...
                        else if( negativeOrNotClamped && currentValue < -threshold )
                        {
                            if( pressedFrame == frameCount )
                            {
                                touchDown = true;
                        ...
                        else if( currentValue <= threshold || currentValue >= -threshold )
                        {
